        ...
        <CardContent>
          <ScrollArea className="h-[600px] pr-4">
            <div className="space-y-1">
              {data.length === 0 ? (
                <div className="text-center py-8 text-sm text-muted-foreground">No telemetry events yet</div>
              ) : (
                data.map((e) => {
                  ...
                  return (
                    <div
                      key={e.id}
                      className="flex items-start gap-3 rounded-lg border border-border/50 bg-card/30 px-3 py-2.5 hover:bg-accent/50 transition-colors"
                    >
                      <div className={`mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-accent/30 ${unavailable ? "text-amber-400" : "text-muted-foreground"}`}>
                        <Icon className="h-3.5 w-3.5" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="text-xs font-medium text-foreground">{e.event_type.replace(/_/g, " ")}</span>
                          <span className={`text-[9px] ${sourceColors[e.source] || "text-muted-foreground"}`}>
                            {e.source}
                          </span>
                          {unavailable && (
