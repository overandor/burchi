    ...
    <div className="space-y-6">
      ...
      {models.length === 0 ? (
        ...
        <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
          {models.map((m) => (
            <Card key={m.model_id} className="border-border bg-card/50">
              <CardHeader className="flex flex-row items-center justify-between">
                <div className="flex items-center gap-2">
                  <Cpu className="h-4 w-4 text-blue-400" />
                  <CardTitle className="text-sm text-foreground">{m.name ?? m.model_id}</CardTitle>
                </div>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-3">
                  <div className="flex flex-col items-center rounded-lg border border-border bg-card/40 p-3">
                    ...
                    <span className="text-[9px] text-muted-foreground/60">parameters</span>
                  </div>
                  <div className="flex flex-col items-center rounded-lg border border-border bg-card/40 p-3">
                  ...
                  <div className="flex flex-col items-center rounded-lg border border-border bg-card/40 p-3">
                    ...
                    <span className="text-[9px] text-muted-foreground/60">arch</span>
                  </div>
                </div>
                ...
                <div className="space-y-1.5 text-xs">
                  <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span className="truncate text-foreground/80">{m.tracker_url ?? "—"}</span>
                  </div>
                  <div className="flex items-center gap-2 text-muted-foreground">
                    <Server className="h-3 w-3 text-muted-foreground/60" />
                    <span className="text-muted-foreground/60">inference:</span>
                    <span className="truncate text-foreground/80">{m.inference_url ?? "—"}</span>
                  </div>
                </div>

                <div className="flex items-center gap-2 border-t border-border pt-3">
                  <span className="text-[10px] uppercase tracking-wider text-muted-foreground/60">merkle</span>
