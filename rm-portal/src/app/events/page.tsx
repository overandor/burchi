          ...
          return (
            <button
              ...
              className={`flex items-center gap-3 rounded-lg border p-4 transition-all ${
                filter === sev
                  ? `${cfg.badge} ring-1 ring-offset-2 ring-offset-zinc-900`
            ...
            >
              ...
              <div className="text-left">
                <div className={`text-2xl font-bold tabular-nums ${filter === sev ? "" : "text-white"}`}>
                  {counts[sev]}
                </div>
                <div className="text-[10px] uppercase tracking-wider text-zinc-500">{sev}</div>
              </div>
            </button>
          )
        })}
      ...
      <Card className="border-zinc-800 bg-zinc-900/50">
        ...
        <CardContent>
          {filteredEvents.length === 0 ? (
            ...
            <ScrollArea className="h-[600px] rounded-lg border border-zinc-800/50">
              <div className="space-y-0.5 p-1">
                {filteredEvents.map((event, idx) => {
                  ...
                  return (
                    <div key={event.id}>
                      <div className="flex items-start gap-3 rounded-lg border border-zinc-800/50 bg-zinc-900/30 px-3 py-2.5 transition-colors hover:bg-zinc-900/60">
                        <Icon className={`mt-0.5 h-3.5 w-3.5 shrink-0 ${cfg.color}`} />
                        <span className="mt-0.5 shrink-0 text-[10px] text-zinc-600 tabular-nums">
                          {new Date(event.timestamp).toLocaleTimeString("en-US", { hour12: false })}
                        </span>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <span className="text-[10px] font-mono text-zinc-400">{event.event_type}</span>
                            <Badge variant="outline" className={`text-[8px] ${cfg.badge}`}>
                              {event.severity.toUpperCase()}
                            </Badge>
                          </div>
                          <p className="mt-0.5 text-xs text-zinc-300">{event.message}</p>
                        </div>
                      </div>
                      {idx < filteredEvents.length - 1 && <Separator className="bg-zinc-800/20" />}
                    </div>
                  )
                })}
              </div>
