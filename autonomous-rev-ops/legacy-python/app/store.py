"""Data store — PostgreSQL (production) or SQLite (local dev)."""
...
def get_telemetry(limit: int = 50, event_type: str = "") -> list[dict]:
    conn = _get_conn()
    if event_type:
        rows = conn.execute("SELECT * FROM telemetry WHERE event_type=? ORDER BY timestamp DESC LIMIT ?", (event_type, limit)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM telemetry ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
...
def update_visitor(visitor_id: str, data: dict) -> Optional[dict]:
    with _lock:
        ...
        if fields:
            ...
            conn.execute(f"UPDATE visitors SET {', '.join(fields)} WHERE visitor_id=?", values)
            conn.commit()
        row = conn.execute("SELECT * FROM visitors WHERE visitor_id=?", (visitor_id,)).fetchone()
...
def list_visitors(limit: int = 50) -> list[dict]:
    conn = _get_conn()
    rows = conn.execute("SELECT * FROM visitors ORDER BY last_seen DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]


def get_high_intent_visitors(limit: int = 20) -> list[dict]:
    conn = _get_conn()
    rows = conn.execute("SELECT * FROM visitors WHERE converted=0 ORDER BY engagement_score DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]
...
def get_experiment(eid: str) -> Optional[dict]:
    ...
    if not row:
        return None
    variants = conn.execute("SELECT * FROM variants WHERE experiment_id=?", (eid,)).fetchall()
...
def list_experiments(limit: int = 20) -> list[dict]:
    ...
    for r in rows:
        exp = dict(r)
        variants = conn.execute("SELECT * FROM variants WHERE experiment_id=?", (r["id"],)).fetchall()
...
def update_variant(vid: str, data: dict) -> None:
    with _lock:
        ...
        if fields:
            values.append(vid)
            conn.execute(f"UPDATE variants SET {', '.join(fields)} WHERE id=?", values)
            conn.commit()
...
def complete_experiment(eid: str, winner_id: str, confidence: float) -> None:
    with _lock:
        conn = _get_conn()
        conn.execute("UPDATE experiments SET status='completed', winner_id=?, confidence=?, ended_at=? WHERE id=?",
...
def update_experiment(eid: str, data: dict) -> None:
    ...
    with _lock:
        conn = _get_conn()
        conn.execute(f"UPDATE experiments SET {sets} WHERE id=?", (*fields.values(), eid))
        conn.commit()
...
def list_content(type: str = "", limit: int = 50) -> list[dict]:
    conn = _get_conn()
    if type:
        rows = conn.execute("SELECT * FROM content_items WHERE type=? ORDER BY created_at DESC LIMIT ?", (type, limit)).fetchall()
    else:
        rows = conn.execute("SELECT * FROM content_items ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]
...
def approve_decision(did: str) -> None:
    with _lock:
        conn = _get_conn()
        conn.execute("UPDATE decisions SET status='approved' WHERE id=?", (did,))
        conn.commit()
...
def count_decisions() -> int:
    conn = _get_conn()
    return conn.execute("SELECT COUNT(*) AS cnt FROM decisions").fetchone()["cnt"]


def count_experiments() -> int:
    conn = _get_conn()
    return conn.execute("SELECT COUNT(*) AS cnt FROM experiments").fetchone()["cnt"]


def count_content() -> int:
    conn = _get_conn()
    return conn.execute("SELECT COUNT(*) AS cnt FROM content_items").fetchone()["cnt"]
...
def get_kpi_history(limit: int = 30) -> list[dict]:
    conn = _get_conn()
    rows = conn.execute("SELECT * FROM kpi_snapshots ORDER BY date DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]
...
def set_control_state(key: str, value: str) -> None:
    with _lock:
        conn = _get_conn()
        conn.execute("INSERT OR REPLACE INTO control_state (key, value) VALUES (?,?)", (key, value))
        conn.commit()
...
def execute_action(aid: str, result: str = "") -> None:
    with _lock:
        conn = _get_conn()
        conn.execute("UPDATE actions SET status='executed', executed_at=?, result=? WHERE id=?", (utc_now(), result, aid))
        conn.commit()
...
def list_live_events(limit: int = 50) -> list[dict]:
    conn = _get_conn()
    rows = conn.execute("SELECT * FROM live_events ORDER BY timestamp DESC LIMIT ?", (limit,)).fetchall()
    return [dict(r) for r in rows]
...
def seed_data() -> None:
    ...
    with _lock:
        conn.execute("UPDATE experiments SET observations=187, confidence=0.81 WHERE id=?", (eid,))
        conn.commit()
