...
export function MicrosoftLogin({ onConnected, onClose }: MicrosoftLoginProps) {
  ...
  const startDeviceFlow = useCallback(async () => {
    ...
    try {
      ...
      setExpiresAt(Date.now() + (data.expires_in - 30) * 1000);
      setStep("show-code");
      ...
      startPolling(data.device_code, data.client_id, data.scopes, data.interval || 5);
    } catch (e: any) {
      setError(e.message);
      setStep("error");
    }
  ...
  const startPolling = useCallback(
    (code: string, cId: string, scps: string[], interval: number) => {
      setStep("polling");
      ...
      pollRef.current = setInterval(async () => {
        ...
        try {
          const res = await fetch("/api/microsoft/token", {
            ...
            body: JSON.stringify({ device_code: code, client_id: cId, scopes: scps }),
          });
          const data = await res.json();
      ...
      }, (interval + 1) * 1000); // +1 to avoid rate limiting
    },
    [expiresAt, onConnected]
  );
      ...
      >
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-blue-200 bg-blue-50 p-4">
              ...
              <p className="mt-1 text-xs text-blue-700">
                If it didn't open, click the button below to open it manually.
              </p>
            </div>
            ...
            <a
              ...
              className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0078d4] px-4 py-3 text-sm font-semibold text-white transition-colors hover:bg-[#106ebe]"
            >
            ...
            {step === "polling" && (
              <div className="flex items-center justify-center gap-2 text-sm text-slate-500">
                <div className="h-4 w-4 animate-spin rounded-full border-2 border-slate-200 border-t-[#0078d4]"></div>
        ...
        {step === "success" && (
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <p className="text-sm font-semibold text-slate-900">Connected successfully!</p>
          </div>
        )}
        ...
        {step === "error" && (
          <div className="space-y-4">
            ...
            <button
              ...
              className="flex w-full items-center justify-center rounded-xl bg-[#0078d4] px-4 py-3 text-sm font-semibold text-white hover:bg-[#106ebe]"
            >
              Try Again
            </button>
          </div>
