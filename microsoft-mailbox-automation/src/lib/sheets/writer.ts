...
export async function exportToExcel(
  ...
  try {
    ...
    console.error("[sheets/writer] exportToExcel error:", e);
    throw new Error(`Failed to export to Excel: ${e instanceof Error ? e.message : String(e)}`);
  }
}
...
export async function exportToCSV(
  ...
  try {
    ...
    for (const record of records) {
      for (const field of record.extractedData.fields) {
        const row = [
          ...
          escapeCsv(String(field.confidence)),
    ...
    console.error("[sheets/writer] exportToCSV error:", e);
    throw new Error(`Failed to export to CSV: ${e instanceof Error ? e.message : String(e)}`);
  }
}
